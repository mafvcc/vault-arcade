...
import { createArkanoidGame, type ArkanoidGame } from "@/lib/games/arkanoid";

export default function ArkanoidPlayer({ game }: { game: Game }) {
  ...
  useEffect(() => {
    ...
    const engine = createArkanoidGame(canvas, {
      onScore: setScore,
      onLives: setLives,
      onLevel: setLevel,
      // Espeja la pausa del motor (tecla P/Escape o botón) en el HUD React.
      onPause: setPaused,
      onGameOver: (finalScore, winFlag) => {
        setScore(finalScore);
        setWon(!!winFlag);
        setOver(true);
      },
      // Cubre el reinicio (restart): cierra el modal y resetea el estado React.
      onPlaying: () => {
        setOver(false);
        setWon(false);
        setSaved(false);
  ...
  const togglePause = () => {
    ...
    // `paused` se actualiza vía el callback onPause del motor.
  };
  ...
  // JUGAR DE NUEVO: reinicia el motor y resetea el estado React (vía onPlaying).
  const restartGame = () => {
    ...
    <div className="av-player av-player--canvas fade-in">
          ...
          {paused && !over && (
            ...
            >
              <div>
                ...
                >
                  PULSA REANUDAR O ELIGE NIVEL EN LA PANTALLA
                </div>
              </div>
            </div>
          )}
      ...
      {over && (
        <div className="modal-bd">
          <div className="modal">
            <h2>{won ? "¡COMPLETASTE EL JUEGO!" : "FIN DEL JUEGO"}</h2>
